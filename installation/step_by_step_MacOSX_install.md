...
You don't need to train your Deep Learning models with a GPU for these LiveLessons, but some of the later notebooks in these LiveLessons will run much more quickly if you do. *Getting the steps below to run may be tricky for someone who's new to working with the Unix command line.* 

1. install CUDA and cuDNN, e.g., per the **Install CUDA** and **CuDNN** sections of [this blog post](https://blog.slavv.com/the-1700-great-deep-learning-box-assembly-setup-and-benchmarks-148c5ebe6415) 
2. in the `TensorFlow-LiveLessons/docker-stack-scripts` directory, run `chmod 777 jupyter_notebook_config.py start*.sh`
